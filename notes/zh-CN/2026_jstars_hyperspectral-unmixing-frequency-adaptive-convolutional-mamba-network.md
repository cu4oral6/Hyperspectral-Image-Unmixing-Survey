...
- **作者**：TBD
- **论文**：https://doi.org/10.1109/JSTARS.2026.3677880
- **代码**：https://github.com/flyzzie/FACM
- **项目页**：TBD
- **本地 PDF**：../../pdfs/Hyperspectral Unmixing Using Frequency-Adaptive Convolutional-Mamba Network.pdf
- **数据集**：Urban
...
- **这篇论文讲了什么**：这是一篇关于线性盲解混; Mamba; 光谱变异的速读笔记，重点记录方法思路、实验设置以及代码线索；相关实验数据集为 Urban。
- **是不是线性盲解混**：是。按本仓库分类，它属于线性盲解混：不预先给定端元光谱，而是从高光谱图像中估计端元和丰度，并以线性/重构式解混设置为主。
- **用了哪些数据集**：Urban。
- **实验效果怎么样**：Urban：报告平均 RMSE 0.1091、平均 SAD 0.1155，低于对比解混基线。

## 一句话总结

这是一篇关于线性盲解混; Mamba; 光谱变异的速读笔记，重点记录方法思路、实验设置以及代码线索；相关实验数据集为 Urban。
