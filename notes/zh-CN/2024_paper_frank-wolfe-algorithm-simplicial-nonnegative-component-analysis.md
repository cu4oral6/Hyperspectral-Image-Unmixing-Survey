...
- **数据集**：synthetic SCA/NCA tests
- **标签**：linear,blind,nmf
- **笔记状态**：基于本地 PDF 元数据和摘要的速读笔记

## 速读要点

- **这篇论文讲了什么**：这是一篇关于线性盲解混; NMF的速读笔记，重点记录方法思路、实验设置以及代码线索；相关实验数据集为 synthetic SCA/NCA tests。
- **是不是线性盲解混**：是。按本仓库分类，它属于线性盲解混：不预先给定端元光谱，而是从高光谱图像中估计端元和丰度，并以线性/重构式解混设置为主。
- **用了哪些数据集**：synthetic SCA/NCA tests。
- **实验效果怎么样**：合成 SCA/NCA 实验：论文报告 Frank-Wolfe 求解器在最小体积分解中具有竞争性表现。

## 一句话总结

这是一篇关于线性盲解混; NMF的速读笔记，重点记录方法思路、实验设置以及代码线索；相关实验数据集为 synthetic SCA/NCA tests。
