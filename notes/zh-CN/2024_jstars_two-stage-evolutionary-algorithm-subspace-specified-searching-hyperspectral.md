...
- **作者**：TBD
- **论文**：https://doi.org/10.1109/JSTARS.2023.3333955
- **代码**：TBD
- **项目页**：TBD
- **本地 PDF**：../../pdfs/Two-Stage Evolutionary Algorithm Based on Subspace Specified Searching for Hyperspectral Endmember Extraction.pdf
- **数据集**：Urban; Cuprite; USGS synthetic
- **标签**：linear,blind,graph
- **笔记状态**：基于本地 PDF 元数据和摘要的速读笔记

## 速读要点

- **这篇论文讲了什么**：这是一篇关于线性盲解混; 图正则的速读笔记，重点记录方法思路、实验设置以及代码线索；相关实验数据集为 Urban; Cuprite; USGS synthetic。
- **是不是线性盲解混**：是。按本仓库分类，它属于线性盲解混：不预先给定端元光谱，而是从高光谱图像中估计端元和丰度，并以线性/重构式解混设置为主。
- **用了哪些数据集**：Urban; Cuprite; USGS synthetic。
- **实验效果怎么样**：在 Urban; Cuprite; USGS synthetic 上，实验与解混基线比较，并报告重构、丰度或端元估计有所改善，评价指标包括 Mean SAD/SAD/RMSE/MSE。

## 一句话总结

这是一篇关于线性盲解混; 图正则的速读笔记，重点记录方法思路、实验设置以及代码线索；相关实验数据集为 Urban; Cuprite; USGS synthetic。
