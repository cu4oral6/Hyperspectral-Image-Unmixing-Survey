...
- **这篇论文讲了什么**：这是一篇关于线性盲解混; 稀疏; 图正则; ADMM; 多尺度的速读笔记，重点记录方法思路、实验设置以及代码线索；相关实验数据集为 论文报告的数据集。
- **是不是线性盲解混**：是。按本仓库分类，它属于线性盲解混：不预先给定端元光谱，而是从高光谱图像中估计端元和丰度，并以线性/重构式解混设置为主。
- **用了哪些数据集**：数据集待补。
- **实验效果怎么样**：数据集待补；已索引 SAD/RMSE 评价，但当前笔记尚未抽取完整数值。

## 一句话总结

这是一篇关于线性盲解混; 稀疏; 图正则; ADMM; 多尺度的速读笔记，重点记录方法思路、实验设置以及代码线索；相关实验数据集为 论文报告的数据集。
