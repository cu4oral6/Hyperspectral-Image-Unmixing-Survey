...
- **出版源**：IEEE IGARSS
- **作者**：TBD
- **论文**：https://doi.org/10.1109/IGARSS55030.2025.11242377
- **代码**：TBD
- **项目页**：TBD
- **本地 PDF**：../../pdfs/Hyperspectral Unmixing Using -2-1 Norm-Based Robust Deep Nonnegative Matrix Factorization.pdf
- **数据集**：Samson; synthetic
- **标签**：linear,blind,nmf
- **笔记状态**：基于本地 PDF 元数据和摘要的速读笔记

## 速读要点

- **这篇论文讲了什么**：这是一篇关于线性盲解混; NMF的速读笔记，重点记录方法思路、实验设置以及代码线索；相关实验数据集为 Samson; synthetic。
- **是不是线性盲解混**：是。按本仓库分类，它属于线性盲解混：不预先给定端元光谱，而是从高光谱图像中估计端元和丰度，并以线性/重构式解混设置为主。
- **用了哪些数据集**：Samson; synthetic。
- **实验效果怎么样**：在 Samson; synthetic 上，实验与解混基线比较，并报告重构、丰度或端元估计有所改善，评价指标包括 SAD/RMSE/MSE。

## 一句话总结

这是一篇关于线性盲解混; NMF的速读笔记，重点记录方法思路、实验设置以及代码线索；相关实验数据集为 Samson; synthetic。
