...
- **出版源**：IEEE IGARSS
- **作者**：TBD
- **论文**：https://doi.org/10.1109/IGARSS55030.2025.11243310
- **代码**：https://github.com/XanderHaijen/two_step_lmm
- **项目页**：TBD
- **本地 PDF**：../../pdfs/A Two-Step Linear Mixing Model for Unmixing Under Hyperspectral Variability.pdf
- **数据集**：DLR HySU
...
- **这篇论文讲了什么**：这是一篇关于线性盲解混; 自编码器; 光谱变异; 注意力的速读笔记，重点记录方法思路、实验设置以及代码线索；相关实验数据集为 DLR HySU。
- **是不是线性盲解混**：是。按本仓库分类，它属于线性盲解混：不预先给定端元光谱，而是从高光谱图像中估计端元和丰度，并以线性/重构式解混设置为主。
- **用了哪些数据集**：DLR HySU。
- **实验效果怎么样**：DLR HySU：在 2LMM 生成光谱变异实验中丰度 RMSE 最优，重构 SAD 与 ELMM 变体接近。

## 一句话总结

这是一篇关于线性盲解混; 自编码器; 光谱变异; 注意力的速读笔记，重点记录方法思路、实验设置以及代码线索；相关实验数据集为 DLR HySU。
